...
RUN apt-get update \
    ...
    && apt-get -y --no-install-recommends install php7.1-bcmath \
    && apt-get -y --no-install-recommends install php7.1-gd \
    && apt-get -y --no-install-recommends install php7.1-zip \
...
RUN sed -i "/listen = .*/c\listen = [::]:9000" /etc/php/7.1/fpm/pool.d/www.conf \
    ...
    && sed -i "/pid = .*/c\;pid = /run/php/php7.2-fpm.pid" /etc/php/7.1/fpm/php-fpm.conf \
